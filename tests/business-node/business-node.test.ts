...
import { buildBusinessNodeFlowPage, buildFlowPage, migrateUserAssets, resolvePendingMigration } from '../../src/index-core/index.js';
...
describe('BusinessNode service', () => {
  it('creates ordered function-only members and supports reorder/collapse', () => {
    ...
    const first = testSnapshot.fragments[0]?.id ?? symbolId('symbol:first');
    ...
    const functionEntry = testSnapshot.fragments[0];
    if (functionEntry === undefined) throw new Error('fixture fragment missing');
    ...
    const staleState: UserWorkspaceState = {
      schemaVersion: 1,
      flowPages: [page, buildFlowPage(testSnapshot, functionEntry.id)],
