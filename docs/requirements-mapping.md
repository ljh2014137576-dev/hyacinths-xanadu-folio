...
| NFR-005 | 文件变化只重建受影响关系 | MVP 0.1 当前 `incrementalUpdate=false`，执行可取消的受控全量重建；partial 只作 transient 预览，completed 才从 last-completed baseline 迁移资产。ADR-0004 将 per-session builder 与 affected relation invalidation 延期至 MVP 0.2 [#17](https://github.com/ljh2014137576-dev/xanadu-code-flow-browser/issues/17)，完成前不得声称增量 | [#4](https://github.com/ljh2014137576-dev/xanadu-code-flow-browser/issues/4)、[#5](https://github.com/ljh2014137576-dev/xanadu-code-flow-browser/issues/5)、[#17](https://github.com/ljh2014137576-dev/xanadu-code-flow-browser/issues/17) |
